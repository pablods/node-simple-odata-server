...
ODataServer.prototype.handle = function (req, res, next) {
    ...
    this.router.dispatch(req, res, next);
};
...
ODataServer.prototype._initializeRoutes = function () {
    ...
    this.router.get("/:collection/\$count/", function(req, res, next) {
        if (next && req.params.collection && !self.cfg.model.entitySets[req.params.collection]) {
            return next();
        }
        req.params.$count = true;
        query(self.cfg, req, res);
    });
    this.router.get("/:collection\\(:id\\)", function(req, res, next) {
        if (next && req.params.collection && !self.cfg.model.entitySets[req.params.collection]) {
            return next();
        }
        query(self.cfg, req, res);
    });
    this.router.get("/:collection", function(req, res, next) {
        if (next && req.params.collection && !self.cfg.model.entitySets[req.params.collection]) {
            return next();
        }
        query(self.cfg, req, res);
    ...
    this.router.post("/:collection", function(req, res, next) {
        if (next && req.params.collection && !self.cfg.model.entitySets[req.params.collection]) {
            return next();
        }
        insert(self.cfg, req, res);
    });
    this.router.patch("/:collection\\(:id\\)", function(req, res, next) {
        if (next && req.params.collection && !self.cfg.model.entitySets[req.params.collection]) {
            return next();
        }
        update(self.cfg, req, res);
    });
    this.router.delete("/:collection\\(:id\\)", function(req, res, next) {
        if (next && req.params.collection && !self.cfg.model.entitySets[req.params.collection]) {
            return next();
        }
