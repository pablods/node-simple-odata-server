...
Node.prototype.transform = function() {
    ...
    if (this.type === "eq" && this.right.type === 'literal') {
        result[this.left.name.replace("/",".")] = this.right.value;
    }

    if (this.type === "lt" && this.right.type === 'literal') {
        result[this.left.name.replace("/",".")] = { "$lt": this.right.value };
    }

    if (this.type === "gt" && this.right.type === 'literal') {
        result[this.left.name.replace("/",".")] = { "$gt": this.right.value };
    }
    ...
    if (this.type === "functioncall") {
        switch (this.func) {
            ...
            case "contains": substringof(this, result); break;
        }
    }
...
function substringof(node, result) {
    ...
    result[prop.name.replace("/",".")] = new RegExp(lit.value, 'i');
}
